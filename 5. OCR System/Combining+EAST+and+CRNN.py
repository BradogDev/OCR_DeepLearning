...
model = cv2.dnn.readNet('../Models/TextDetection/frozen_east_text_detection.pb')
model1 = cv2.dnn.readNet('../Models/TextRecognition/crnn.onnx')

#Crear un directorio temporal en la raíz del proyecto si no existe
...
alphabet_set = "0123456789abcdefghijklmnñopqrstuvwxyz"
blank = '-'
...
img_copy = img.copy()
for idx, (x1, y1, x2, y2) in enumerate(fin_boxes):
    ...
    temp_image_path = os.path.join(temp_dir, f"temp_{text.strip()}.jpg")
    cv2.imwrite(temp_image_path, segment)
