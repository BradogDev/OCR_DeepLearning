...
def DetectLicenseInCar(gray):
    ...
    #cv2.imshow('plates CLAHE',gray_img_clahe)
    #cv2.waitKey()
    ...
    if len(TabCordsLicensesDetected) > 0:
        #print(License[i] + "  detected with CLAHE and THRESH_BINARY" )
        #print(License + "  detected with CLAHE and THRESH_BINARY" )
        for i in range(len(TabCordsLicensesDetected)):
...
####
    ...
    for w in range(len(TabLicensesDetected)):
        ...
        # INV options are not considered as International licenses plates are not INV
        ...
        if Detect_International_LicensePlate(text)== 1:
            ...
            if text==Licenses[i]:
               ...
               TotHits=TotHits+1
                ...
                print(Licenses[i] + " detected as "+ text) 
        ################################################################
        # Filters eliminated to reduce time
        ################################################################
        ####################################################
        # experimental formula based on the brightness
        # of the whole image 
    ...
    """     
    #print("SALEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE")  
...
with open( "LicenseResults.txt" ,"w") as  w:
    ...
    for i in range (len(imagesComplete)):
            ...
            if Licenses[i] < LicenseFrom:
            #  print ("Salta "+ Licenses[i]) 
               continue
            if Licenses[i] > LicenseTill:
            #  print ("Salta "+ Licenses[i]) 
               continue
            Cont=Cont+1
